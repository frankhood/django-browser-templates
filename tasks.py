...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-browser-templates runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-browser-templates.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ skels_manager")
...
def lint(c):
    ...
    """
    c.run("flake8 django-browser-templates tests")
...
def release(c, bumpsize=''):
    ...
    import django_browser_templates
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_browser_templates.__version__))
    c.run("git push --tags")
